...
$('#getWeatherBtn').click(() => {
    ...
    $.ajax ({
        ...
        success :(data) => {
            ...
            // Create a Map with key as day number
            const dateMap = {
                // 0: [],
                // 1: []
            };
            data.list.forEach(date => {
                const dateCorrespondingToDate = new Date(date.dt*1000).getDay();
                if (dateCorrespondingToDate in dateMap) {
                    dateMap[dateCorrespondingToDate].push(date);
                } else {
                    const arr = [];
                    arr.push(date);
                    dateMap[dateCorrespondingToDate] = arr;
                }
            })
            console.log(dateMap);
           /* dateMap.forEach(function(day) {
               const currentDay =  moment(day.dt*1000).format("dddd");
